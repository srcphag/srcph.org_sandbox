...
function patchFinishedLoading(patch) {
  ...
  if (pageContext == "index") {
    ...
    if (mq1.matches) {
      // Function to check if an element is visible in a specific screen area
      // function isElementInViewArea(element, areaTop, areaBottom) {
      //   const rect = element.getBoundingClientRect();
      ...
      //   const targetTop = viewportHeight * (areaTop / 100);
      //   const targetBottom = viewportHeight * (areaBottom / 100);

      //   // Check if element's top or bottom edge is within the target area
      //   const isVisible =
      //     (rect.top >= targetTop && rect.top <= targetBottom) ||
      //     (rect.bottom >= targetTop && rect.bottom <= targetBottom) ||
      //     (rect.top <= targetTop && rect.bottom >= targetBottom);

      //   return isVisible;
      // }
      /////////////////////////////////////////////////////////////////////////////

      function getMostVisibleElement(elements, areaTop, areaBottom) {
        ...
        elements.forEach((element) => {
          ...
          const intersectionTop = Math.max(rect.top, targetTop);
          const intersectionBottom = Math.min(rect.bottom, targetBottom);
          const intersectionHeight = Math.max(
            0,
            intersectionBottom - intersectionTop
          );

          if (intersectionHeight > maxVisibleArea) {
      ...
      //////////////////////////
      function updateVisibleElement(areaTop = 0, areaBottom = 100) {
        const workElements = document.querySelectorAll(".work");
        let previousVisible = document.querySelector(".work.visible");
        const visibleElement = getMostVisibleElement(
          Array.from(workElements),
          areaTop,
          areaBottom
        );

        if (visibleElement) {
          if (!previousVisible || visibleElement !== previousVisible) {
            // Scroll with specific vertical positioning
            // gsap.to(window, {
            //   duration: 0.2,
            //   scrollTo: {
            //     y: visibleElement,
            //     offsetY: 65, // pixels from the top of the viewport
            ...
            visibleElement.classList.add("visibleImage");

            var titleElement = visibleElement.querySelector(".title");
            titleElement.classList.add("visibleTitle");

            var descriptionElement =
              visibleElement.querySelector(".description");
            descriptionElement.classList.add("visibleDescription");

            mainColor = visibleElement.getAttribute("color");
            CABLES.patch.setVariable("mainColorHex", mainColor);
            const videoElement = visibleElement.querySelector(".videojs-work");
            if (!videoElement) return;
            const player = videojs(videoElement);
            player.muted(true);
            player.play();
        ...
        () => {
          ...
          scrollTimeout = setTimeout(() => {
            if (!isScrolling) {
              ...
              setTimeout(() => {
                isScrolling = false;
              }, 500); // Match this with GSAP duration
            }
          }, 1);
        },
        { passive: true }
      ...
      // Don't forget to register GSAP plugins
    }
  }
